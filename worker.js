...
<body class="min-h-screen font-sans text-slate-800 dark:text-slate-100 selection:bg-emerald-200 dark:selection:bg-emerald-900 transition-colors duration-300">
    ...
    // ---------- Sun-Panel 导入兼容 ----------
    function isSunPanelFormat(obj) {
        return obj && obj.appName === 'Sun-Panel-Config' && Array.isArray(obj.icons);
    }

    function parseSunPanel(obj) {
        const categories = {};
        for (const group of obj.icons) {
                ...
                reader.onload = async (event) => {
                    try {
                        ...
                        if (trimmed.startsWith('<!DOCTYPE') || /<(DL|H3)\b/i.test(trimmed)) {
                            ...
                            // 检测 Sun-Panel 格式并转换
                            if (isSunPanelFormat(data)) {
                                data = parseSunPanel(data);
                                if (!data) throw new Error("Sun-Panel 文件中未找到有效书签");
                            }
                        }
