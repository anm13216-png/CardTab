import { getIconApi, isPreferIconApi } from '../utils/config.js';
...
export async function handleIconProxy(request, ctx) {
    ...
    if (response) {
        ...
        if (isPreferIconApi()) {
            const upstreamApi = `${getIconApi()}${encodeURIComponent(targetUrl)}`;
