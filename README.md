...
## 📢 版本说明与最新更新
> 💡 **版本规范标注**：本项目自 **vr1.0.1** 起引入版本命名规范 —— **`vr` 开头为正式版本（Release）**，**`v` 开头为测试版本（Beta / Dev）**。

### 🚀 最新正式版 (v1.0.2.10)
- 🐛 **部署与配置修复**：修复 GitHub Actions 部署过程中 wrangler.toml 键名重复导致的 Can't redefine existing key 部署中断问题。
- 🔐 **验证逻辑优化**：优化超级管理员与 Cloudflare 环境变量密码绑定与匹配机制。

### 🚀 历史版本 (vr1.0.2)
- 👥 **多用户管理与权限隔离（对标 Sun-Panel 逻辑）**：
  - **三级权限与隔离**：支持超级管理员 (super_admin)、管理员 (admin) 与普通用户 (user)。超级管理员与各管理员之间数据隔离，管理员与所属普通用户共享数据。
  - **密码可视化**：超级管理员可在后台直接查看所有账号的用户名与密码。
- 🌐 **未登录默认界面与发布**：
  - 未登录访客仅可查看初始化默认界面。
  - 仅超级管理员可编辑并一键点击「🌐 发布为未登录默认界面」。
- 🔐 **登录框与安全优化**：
  - 移除默认登录预填 admin 账号选项，改由手动输入。
  - 「👤 设置」下拉菜单新增「👥 账号管理」快捷功能。

### 🚀 历史版本 (vr1.0.1)
- 👤 **完整用户名 + 密码登录支持**：身份验证弹窗正式补全「管理员账号」与「访问密码」双输入框。消除仅传密码导致的自定义环境变量 `ADMIN_USERNAME` 校验失败 Bug。
- ⌨️ **回车快捷登录与焦点体验**：账号/密码框支持按 `Enter` 快捷提交，弹窗打开时智能聚焦与预置，大幅提升登录流畅度。
- 🔑 **JWT 双令牌与时序安全防护**：基于 Access Token + Refresh Token 双令牌机制及 `timingSafeStringEqual` 恒定时间比较，结合服务端世代号 (`__keygen__`) 支撑一键完全安全登出与 Token 吊销。
- 🐛 **组件响应与下拉菜单修复**：彻底修复从单文件拆分至独立 `index.html` 时因 JavaScript 正则表达式转义引起的语法报错，恢复「本站」下拉菜单、「👤 设置」菜单与分类点击交互。
- 🌟 **开箱即用默认书签 (Seed Data)**：内置基础导航书签与 Empty State 美化引导，解决全新部署时主屏空白无数据引导的问题。
